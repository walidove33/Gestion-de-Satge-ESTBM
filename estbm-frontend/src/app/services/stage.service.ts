...
export class StageService {
  ...
  /**
   * Créer une demande de stage (sans fichiers)
   */
  ...
  return this.http.post<Stage>(`${this.baseUrl}/demande`, data, { headers })
    .pipe(catchError(this.handleError));
}
  ...
  downloadAssurance(stageId: number): Observable<Blob> {
    return this.http.get(`${this.baseUrl}/${stageId}/documents`, { responseType: 'blob' })
      .pipe(catchError(this.handleError));
  }
  ...
  approveStage(stageId: number, note?: string): Observable<Stage> {
    return this.http.put<Stage>(`${this.baseUrl}/encadrant/${stageId}/valider`, { note: note || '' })
      .pipe(catchError(this.handleError));
  }

  rejectStage(stageId: number, note: string): Observable<Stage> {
    return this.http.put<Stage>(`${this.baseUrl}/encadrant/${stageId}/refuser`, { note })
      .pipe(catchError(this.handleError));
  }

  addNote(stageId: number, note: string): Observable<Stage> {
    return this.http.put<Stage>(`${this.baseUrl}/encadrant/${stageId}/note`, { commentaire: note })
      .pipe(catchError(this.handleError));
  }
  ...
  validateRapport(rapportId: number, commentaire?: string): Observable<Rapport> {
    return this.http.put<Rapport>(`${this.baseUrl}/encadrant/rapports/${rapportId}/validate`, { commentaire: commentaire || '' })
      .pipe(catchError(this.handleError));
  }
  ...
  /**
   * Upload de l'assurance et de la convention pour un stage (Admin)
   */
  uploadStageDocuments(
    ...
    return this.http.put(
      `${this.adminUrl}/${stageId}/documents`,
      formData
    ).pipe(catchError(this.handleError));
  }
  ...
  private handleError = (error: any): Observable<never> => {
    console.error('StageService error:', error);
    let errorMessage = 'Une erreur est survenue';
    if (error.status === 401) errorMessage = 'Session expirée. Veuillez vous reconnecter.';
